...
class TestRightAngleCurvedCrossSection:
    ...
    def test_polygon(self, cross_section: RightAngleCurvedCrossSection) -> None:
        """Test the geometry property of the RightAngleCurvedCrossSection class."""
        polygon = cross_section.polygon
        assert polygon.is_valid
        assert polygon.area == pytest.approx(expected=cross_section.area, rel=1e-3)
    ...
    def test_mirrored_geometry(self) -> None:
        ...
        cross_section_mirrored_horizontally = RightAngleCurvedCrossSection(radius=50.0, x=0.0, y=0.0, mirrored_horizontally=True)
        assert cross_section_mirrored_horizontally.polygon.centroid.x == pytest.approx(expected=-cross_section.polygon.centroid.x, rel=1e-3)
        assert cross_section_mirrored_horizontally.area == pytest.approx(expected=cross_section.area, rel=1e-3)
        assert cross_section_mirrored_horizontally.centroid.x == pytest.approx(expected=-cross_section.centroid.x, rel=1e-3)
        assert cross_section_mirrored_horizontally.centroid.y == pytest.approx(expected=cross_section.centroid.y, rel=1e-3)

        # Test flipped vertically
        cross_section_mirrored_vertically = RightAngleCurvedCrossSection(radius=50.0, x=0.0, y=0.0, mirrored_vertically=True)
        assert cross_section_mirrored_vertically.polygon.centroid.y == pytest.approx(expected=-cross_section.polygon.centroid.y, rel=1e-3)
        assert cross_section_mirrored_vertically.area == pytest.approx(expected=cross_section.area, rel=1e-3)
        assert cross_section_mirrored_vertically.centroid.x == pytest.approx(expected=cross_section.centroid.x, rel=1e-3)
        assert cross_section_mirrored_vertically.centroid.y == pytest.approx(expected=-cross_section.centroid.y, rel=1e-3)

        # Test flipped both horizontally and vertically
        cross_section_mirrored_both = RightAngleCurvedCrossSection(radius=50.0, x=0.0, y=0.0, mirrored_horizontally=True, mirrored_vertically=True)
        assert cross_section_mirrored_both.polygon.centroid.x == pytest.approx(expected=-cross_section.polygon.centroid.x, rel=1e-3)
        assert cross_section_mirrored_both.polygon.centroid.y == pytest.approx(expected=-cross_section.polygon.centroid.y, rel=1e-3)
        assert cross_section_mirrored_both.area == pytest.approx(expected=cross_section.area, rel=1e-3)
        assert cross_section_mirrored_both.centroid.x == pytest.approx(expected=-cross_section.centroid.x, rel=1e-3)
        assert cross_section_mirrored_both.centroid.y == pytest.approx(expected=-cross_section.centroid.y, rel=1e-3)
    ...
    def test_section_properties(self, cross_section: RightAngleCurvedCrossSection) -> None:
        """Test the section properties of the RectangularCrossSection class."""
        section_properties = cross_section.section_properties()
        assert section_properties.mass == pytest.approx(expected=cross_section.area, rel=1e-2)
