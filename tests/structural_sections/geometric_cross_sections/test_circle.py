...
class TestCircularCrossSection:
    ...
    def test_polygon(self, circular_cross_section: CircularProfile) -> None:
    ...
    def test_radius(self, circular_cross_section: CircularProfile) -> None:
    ...
    def test_geometry_bounds(self, circular_cross_section: CircularProfile) -> None:
    ...
    def test_no_plotter_defined(self, circular_cross_section: CircularProfile) -> None:
    ...
    def test_immutability(self, circular_cross_section: CircularProfile) -> None:
        """Test that the CircularCrossSection dataclass is immutable."""
        with pytest.raises(AttributeError):
            circular_cross_section.name = "New Name"  # type: ignore[misc]

    def test_transform(self, circular_cross_section: CircularProfile) -> None:
        """Test the transform method of the CircularCrossSection class."""
        transformed_section = circular_cross_section.transform(horizontal_offset=10.0, vertical_offset=20.0, rotation=90.0)
        assert isinstance(transformed_section, CircularProfile)
        assert pytest.approx(transformed_section.centroid.x, rel=1e-6) == circular_cross_section.centroid.x + 10.0
        assert pytest.approx(transformed_section.centroid.y, rel=1e-6) == circular_cross_section.centroid.y + 20.0
        assert pytest.approx(transformed_section.profile_height, rel=1e-6) == circular_cross_section.profile_width
