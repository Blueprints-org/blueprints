"""Testing the fatigue life curve value from EN 1993-1-9:2005: Chapter 7 - Fatigue strength."""
...
class TestForm7FatigueLifeCurveValue:
    """Validation for the fatigue life curve value from EN 1993-1-9:2005."""

    @pytest.mark.parametrize(
        ("delta_sigma_ref", "n_ref", "m", "delta_sigma_r", "point", "stress_type"),
        [
            (160.0, 2e6, 3.0, 201.587, "C", StressType.DIRECT),  # N_R = N_C (Δσ_C/Δσ_R)^m, first branch
            (117.889, 5e6, 5.0, 89.343, "D", StressType.DIRECT),  # N_R = N_D (Δσ_D/Δσ_R)^m, second branch
            (100.0, 2e6, 5.0, 114.870, "C", StressType.SHEAR),  # N_R = N_C (Δτ_C/Δτ_R)^m, single shear branch
        ],
    )
    def test_evaluation(
        self, delta_sigma_ref: float, n_ref: float, m: float, delta_sigma_r: float, point: Literal["C", "D"], stress_type: StressType
    ) -> None:
        """Test the evaluation of the result for all branch variants."""
        form = Form7FatigueLifeCurveValue(
            delta_sigma_ref=delta_sigma_ref, n_ref=n_ref, m=m, delta_sigma_r=delta_sigma_r, point=point, stress_type=stress_type
        )
    ...
    def test_reference_stress_maps_to_reference_cycles(self) -> None:
        ...
        form = Form7FatigueLifeCurveValue(delta_sigma_ref=160.0, n_ref=2e6, m=3.0, delta_sigma_r=160.0, point="C", stress_type=StressType.DIRECT)
    ...
    def test_raise_error_for_shear_d_point(self) -> None:
        """The shear curve (Figure 7.2) has a single branch anchored at C, so point 'D' is undefined for it."""
        with pytest.raises(ValueError, match="Invalid point"):
            Form7FatigueLifeCurveValue(delta_sigma_ref=100.0, n_ref=2e6, m=5.0, delta_sigma_r=100.0, point="D", stress_type=StressType.SHEAR)
    ...
    def test_raise_error_if_less_or_equal_to_zero(self, delta_sigma_ref: float, n_ref: float, m: float, delta_sigma_r: float) -> None:
        ...
        with pytest.raises(LessOrEqualToZeroError):
            Form7FatigueLifeCurveValue(
                delta_sigma_ref=delta_sigma_ref, n_ref=n_ref, m=m, delta_sigma_r=delta_sigma_r, point="C", stress_type=StressType.DIRECT
            )
    ...
    def test_latex(self, point: Literal["C", "D"], stress_type: StressType, inputs: dict[str, float], representation: str, expected: str) -> None:
        """Test the latex representation on both branches, including the Δσ (direct) vs Δτ (shear) symbol switch."""
        latex = Form7FatigueLifeCurveValue(**inputs, point=point, stress_type=stress_type).latex()  # type: ignore[arg-type]
