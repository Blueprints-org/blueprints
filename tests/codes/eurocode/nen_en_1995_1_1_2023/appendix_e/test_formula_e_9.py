"""Testing Formula E.9 from NEN-EN 1995-1-1:2023."""
...
from blueprints.codes.eurocode.nen_en_1995_1_1_2023.appendix_e.formula_e_9 import FormEDot9BendingStressInILayer
from blueprints.validations import LessOrEqualToZeroError, NegativeValueError


class TestFormEDot9BendingStressInILayer:
    """Validation for formula E.9 from NEN-EN 1995-1-1:2023."""

    @pytest.mark.parametrize(
        ("n_i", "e_i", "h_i", "m_yd", "ei_ef", "expected_result"),
        [
            (2, 1, 1, 1, 1, 0.5),
            (1, 12000, 70, 10e+06, 6015486104925.15, 0.6981979),
            (2, 4000, 20, 10e+6, 6.2e+12, 0.064516129),
            (3, 12000, 60, -8e+6, 5.5e+12, -0.523636),
            (2, 2000, 30, 0, 1e+12, 0),
        ],
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (3, 12000, 60, -8e+6, 5.5e+12, does_not_raise()),
            (3, 1, 0, 1, 1, does_not_raise()),
            (3, -12000, 60, 8e+6, 5.5e+12, pytest.raises(LessOrEqualToZeroError)),
            (1, 12000, -60, 8e+6, 5.5e+12, pytest.raises(LessOrEqualToZeroError)),
            (3, 12000, -60, 8e+6, 200, pytest.raises(NegativeValueError)),
            (4, 12000, 60, 8e+6, 20, pytest.raises(ValueError, match="The number of the layer must be either 1, 2 or 3")),
            (0, 12000, 60, 8e+6, 20, pytest.raises(ValueError, match="The number of the layer must be either 1, 2 or 3")),
            (3, 12000, 60, 8e+6, -20, pytest.raises(LessOrEqualToZeroError)),
        ],
        ids=["passes1",
             "passes2",
             "passes-h3=0",
