...
class TestNominalConcreteCover:
    ...
    def test_latex_representation(self) -> None:
        ...
        assert (
            ...
            r"\text{Comparison of nominal cover and minimum cover with regard to casting surface}  \newline "
            r"\max \left\{c_{nom}; c_{min,\text{casting surface}}\right\}  \newline "
            r"= \max \left\{55.0; 110.0\right\} = 110.0 \ mm \newline "
            r"\text{Where according to art. 4.4.1:}  \newline "
            r"c_{nom} = c_{min,total}+\Delta c_{dev} = 45.0+10 = 55.0 \ mm \newline "
            r"\text{According to art. 4.4.1.3 (1):}  \newline "
            r"c_{min,total} = c_{min} + \Delta c_{uneven surface} + \Delta c_{abrasion class} = 35.0 + 5 + 5 = 45.0 \ mm \newline "
            r"\text{According to art. 4.4.1.2 (11) and (13):} \newline "
            r"c_{min} = \max \left\{c_{min,b}; c_{min,dur}+\Delta c_{dur,\gamma}-\Delta c_{dur,st}-\Delta c_{dur,add}; 10 \text{mm}\right\} \newline "
            r"= \max \left\{30.0; 35.0+0-0-0; 10\right\} = 35.0 \ mm \newline "
            r"\text{According to art. 4.4.1.2 (6), (7), and (8):} \newline "
            r"c_{min,b} = (equivalent) \ rebar \ diameter + 5 = 25 + 5 = 30 \ mm \newline "
            r"\text{According to table 4.2, structural class S4 & exposure classes (XC1, XD1, XS1) becomes:} \newline "
            r"c_{min,dur} = 35 \ mm \newline "
            r"\text{According to table 4.4:}  \newline "
            r"k2 \ge c_{min,dur} + 75 mm \ for \ Directly \ against \ soil"
        )
