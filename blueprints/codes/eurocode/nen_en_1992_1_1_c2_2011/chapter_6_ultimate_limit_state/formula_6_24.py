...
class Form6Dot24DesignShearStress(Formula):
    r"""Class representing formula 6.24 for the calculation of the design shear stress in the interface, :math:`v_{Edi}`."""
    ...
    def __init__(
        ...
        ----------
        beta : DIMENSIONLESS
            [:math:`\beta`] Ratio of the longitudinal force in the new concrete area and the total longitudinal force either in
            the compression or tension zone, both calculated for the section considered [-].
        v_ed : KN
            [:math:`V_{Ed}`] Transverse shear force [:math:`N`].
        z : MM
            [:math:`z`] Lever arm of composite section [:math:`mm`].
        b_i : MM
            [:math:`b_{i}`] Width of the interface (see Figure 6.8) [:math:`mm`].
        """
        super().__init__()
        self.beta = beta
        self.v_ed = v_ed
