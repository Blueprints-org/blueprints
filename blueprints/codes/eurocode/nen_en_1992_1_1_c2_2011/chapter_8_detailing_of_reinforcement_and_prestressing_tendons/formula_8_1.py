...
class Form8Dot1RequiredMinimumMandrelDiameter(Formula):
    ...
    def __init__(
        ...
        r"""[$$\Ø_{m,min}$$] minimum mandrel diameter if it needs to be checked to avoid concrete failure [$$\text{MM}$$].
        ...
        ----------
        f_bt: KN
            [$$F_{bt}$$] Tensile force from ultimate loads in a bar or group of bars in contact at the start of a bend  [$$\text{kN}$$].
        a_b: MM
            ...
            For a bar or group of bars adjacent to the face of the member, [$$a_b$$] should be taken as the cover plus [$$\Ø/2$$] [$$\text{mm}$$].
        diameter: MM
            [$$\Ø$$] Diameter of reinforcing bar [$$\text{mm}$$].
        f_cd: MPA
            [$$f_{cd}$$] Design value of concrete compressive stress [$$\text{MPa}$$].
