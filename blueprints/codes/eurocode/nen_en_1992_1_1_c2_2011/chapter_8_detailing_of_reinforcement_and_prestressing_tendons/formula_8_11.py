...
class Form8Dot11MinimumDesignLapLength(Formula):
    ...
    def __init__(
        ...
        alpha_6 : DIMENSIONLESS
            ...
            $(ρ_l/25)^{0.5} <= 1.5$ with a minimum of 1.0.

            Where: $ρ_l$ reinforcement percentage lapped within $0.65 ⋅ l_0$ from the centre of the lap length
            considered (see figure 8.8) [-].

            Use your own implementation for this value or use the :class:`SubForm8Dot10Alpha6` class.
        l_b_rqd: MM
            [$l_{b,rqd}$] Basic required anchorage length, for anchoring the force $A_s ⋅ σ_{sd}$ in a straight bar assuming constant
