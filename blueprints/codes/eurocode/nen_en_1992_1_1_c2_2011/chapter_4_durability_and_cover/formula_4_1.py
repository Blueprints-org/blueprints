...
class Form4Dot1NominalConcreteCover(Formula):
    ...
    def latex(self) -> LatexFormula:
        ...
        latex_delta_c_dev = variable_with_subscript(r"\Delta c", "dev")
        return LatexFormula(
            return_symbol=variable_with_subscript("c", "nom"),
            result=to_text(self),
            equation=f"{variable_with_subscript('c', 'min')}+{latex_delta_c_dev}",
            numeric_equation=f"{to_text(self.c_min)}+{to_text(self.delta_c_dev)}",
