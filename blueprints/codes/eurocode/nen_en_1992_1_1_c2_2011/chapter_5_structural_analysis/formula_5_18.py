...
class Form5Dot18ComparisonGeneralSecondOrderEffects:
    ...
    def __init__(self, f_ved: KN, k_1: DIMENSIONLESS, n_s: DIMENSIONLESS, length: M, e_cd: MPA, i_c: MM4) -> None:
        ...
        ----------
        f_ved : KN
            [:math: F_{v,ed}`] Total vertical load (on braced and bracing members) [$kN$].
        k_1 : DIMENSIONLESS
        ...
        length : M
            [:math: `L`] is the total height of the building above level of moment restraint. [$m$].
        e_cd : MPa
            [:math: `E_{cd}`] is the design value of the modulus of elasticity of concrete. [$MPa$].
        i_c : MM4
            [:math: `l_c`] is the second moment of area (uncracked concrete section) of bracing member(s). [$mm^4$].
        """
        self.f_ved = f_ved
        self.k_1 = k_1
        self.n_s = n_s
