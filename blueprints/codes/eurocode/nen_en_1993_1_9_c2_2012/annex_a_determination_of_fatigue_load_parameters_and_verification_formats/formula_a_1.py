...
class FormADot1DamageDuringDesignLife(Formula):
    ...
    def __init__(self, n_e: list[DIMENSIONLESS], n_r: list[DIMENSIONLESS]) -> None:
        ...
        n_e : list[DIMENSIONLESS]
            [$n_E$] Contains number of cycles associated with the stress range γ_Ff·Δσ_i for each band i in the factored spectrum [$-$].
        n_r : list[DIMENSIONLESS]
            [$N_R$] Contains the endurance (in cycles) obtained from the factored Δσ_C/γ_Mf-n_r curve for each stress range of
            γ_Ff·Δ_i [$-$]
