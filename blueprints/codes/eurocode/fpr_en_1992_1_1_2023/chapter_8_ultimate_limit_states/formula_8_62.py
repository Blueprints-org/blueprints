...
class Form8Dot62EnhancedShearStressResistanceInclinedShearReinforcement(Formula):
    ...
    def __init__(
        ...
        f_ywd : MPA
            ...
            inclinations with [$\cot\theta < \tan(\alpha_w/2)$] it is replaced by the stress [$\sigma_{swd}$]
            according to Formula (8.63), see Form8Dot63StressInInclinedShearReinforcement [$MPa$].
        alpha_w : DEG
    ...
    def _evaluate(
        ...
        cot_alpha_w = 1 / np.tan(alpha_w_rad)

        # The denominator 1 + cot^2(theta) is never zero, so it needs no guard of its own.
        enhanced = nu * f_cd * (cot_theta - cot_beta_incl) / (1 + cot_theta**2) + rho_w * f_ywd * (cot_beta_incl + cot_alpha_w) * np.sin(alpha_w_rad)
