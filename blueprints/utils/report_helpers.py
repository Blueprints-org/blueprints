...
class ReportHelpers:
    ...
    @staticmethod
    def add_calculation_formula(report: Report, calculation_formula: dict[str, Formula], n: int = 2) -> None:
        """
        Add calculation steps formula to the report.

        report : Report
            The report object to which the calculation steps will be added.
        calculation_formula : dict[str, Formula]
            Iterable of (step_name, step_instance) tuples.
        n : int, optional
            Number of decimals (default is 2).
        """
        steps = list(calculation_formula.items()) if isinstance(calculation_formula, dict) else list(calculation_formula)
        for _, step in steps:
