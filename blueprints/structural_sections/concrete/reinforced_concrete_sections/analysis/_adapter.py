...
from concreteproperties.results import CrackedResults, MomentCurvatureResults, StressResult
...
from blueprints.type_alias import DEG, DIMENSIONLESS, MM, MPA
...
class AnalysisLevel(Enum):
    ...
    axis (see :func:`_cracked_stress_with_axial`). The analyzer caches one backend section per
    configuration, so only the sections needed for the requested analyses are ever built. A future ULS
    level with design materials (f_yd, f_cd) slots in here.
    """
...
def build_concrete_section(
    ...
    profile's bounding-box height. The ``level`` selects the concrete material model: linear
    (tension-carrying) for SLS, no-tension for SLS_CRACKED.
    ...
    level : AnalysisLevel
        The material model level (SLS or SLS_CRACKED).
    elastic_modulus : MPA | None
        The concrete elastic modulus for the service profile: the effective modulus E_c,eff when
        analyzing with creep, or ``None`` for the short-term secant modulus e_cm.
