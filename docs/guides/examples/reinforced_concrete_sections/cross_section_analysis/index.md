...
**Analyse a reinforced-concrete cross-section straight from its internal forces.** Give `CrossSectionAnalysis` a section (rectangular, circular or custom) and a set of `SectionForces`, and it returns the serviceability stresses and strains — in one consistent, plottable API.

!!! tip "Where should you start?"

    - **New to this?** Read the short [mental model](#the-mental-model) and [conventions](#coordinate-system-signs-and-units) below, run the [60-second quick start](#the-60-second-quick-start), then work through the [SLS](sls.md) page.
    - **Know what you need?** Jump straight to the [task → method map](#task-method-map) and pick the one call you're after.

!!! info "Scope of this release"

    This guide covers the **serviceability** limit state. The ultimate limit state — bending capacity, N-M and biaxial interaction diagrams, moment-curvature and the unity check — is a follow-up on the same analyzer and the same `SectionForces` input, so nothing here changes when it lands.
...
(geometry + materials +      (the analyzer)             (stresses, strains,
 longitudinal rebars)                                    cracked properties)
```

The question this guide answers is the **SLS** one — *"what are the stresses and strains under this everyday load?"* Linear-elastic, with an automatic cracked/uncracked decision. This is the [SLS page](sls.md).
...
| θ (`theta`) | neutral-axis angle of the cracked state | ° |
...
!!! note "What `theta` means"
    ...
    `theta` is the **neutral-axis angle** — the orientation of the zero-strain line — not the direction of the moment vector. A horizontal neutral axis (`theta = 0`) belongs to bending about the y-axis: sagging, with tension at the bottom.
...
Build the beam, then ask the analyzer for the stress/strain state under a set of section forces:
...
print(f"Reinforcement stress:  {sls.rebar_results[0].stress:6.1f} MPa")
print(f"Neutral axis depth:    {sls.cracked_properties.neutral_axis_depth:6.1f} mm")
```
...
| the cracked-section properties (`M_cr`, neutral axis, `I_cr`) | `stress(forces).cracked_properties` | `CrackedProperties` | [SLS](sls.md#cracked-section-properties) |
| the backend's 2D mesh stress contour | `stress(forces).plot_mesh_stress()` | matplotlib `Axes` | [SLS](sls.md#backend-mesh-contour) |
| to drop the cached backend sections after mutating the section | `invalidate_cache()` | — | — |
...
- **The ultimate limit state is a follow-up.** Bending capacity, interaction diagrams, moment-curvature and the unity check are not part of this release.
...
# long-term state, with creep through the effective modulus
long_term = analysis.stress(forces, creep_coefficient=2.0)
print(f"Long-term steel stress: {long_term.rebar_results[0].stress:.1f} MPa")

# visualise the SLS strain/stress state
sls.plot().show()
```

**Ready?** Start with the [SLS analysis](sls.md).
